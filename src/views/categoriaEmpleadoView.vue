<template>
    <v-container>
        <v-card>
            <v-card-title>Gestion de Categorias de Empleados</v-card-title>
            <v-card-text>
                <v-form @submit.prevent="guardarCategorias">
                    <v-text-field
                        v-model="form.clave"
                        label="Clave"
                        required
                ...
                >
                    <template v-slot:item.actions="{ item }">
                        ...
                        <v-btn icon @click="eliminarCategoria(item.id)">
...
export default {
    ...
    methods: {
        ...
        try {
            if (this.modoEditar) {
            await axios.put(`http://localhost:3000/categoriaempleado/${this.form.id}`, {
        ...
        try {
            await axios.delete(`http://localhost:3000/categoriaempleado/${id}`);
            this.obtenerCategorias();
        } catch (err) {
